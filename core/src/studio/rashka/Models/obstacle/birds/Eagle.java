...
public class Eagle implements Collides {
    ...
    private Rectangle bodyEagle, wings;
    ...
    public Eagle() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("eagle")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("eagle")), 17, 0.7f);
        ...
        bodyEagle = new Rectangle(position.x + 15, position.y + 38, 80, 15);
    ...
    public void update(float deltaTime) {
        if (isGo) {
            if (position.x < RuDragonGame.WIDTH && isSound) {
                if (RuDragonGame.getPreference().loadSound() == 1) RuDragonGame.getMusicSound().sounds.get("eagle").play();
                isSound = false;
            }
            ...
            bodyEagle.setPosition(position.x + 15, position.y + 38);
    ...
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyEagle) || pawsR11.overlaps(bodyEagle) || pawsR12.overlaps(bodyEagle) ||
                paws.overlaps(bodyEagle) || head.overlaps(bodyEagle) || wings.overlaps(bodyEagle) ||
