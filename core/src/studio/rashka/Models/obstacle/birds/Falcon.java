...
public class Falcon implements Collides {
    ...
    private Rectangle bodyFalcon, wings;
    ...
    public Falcon() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("falcon")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("falcon")), 17, 0.7f);
        ...
        bodyFalcon = new Rectangle(position.x + 15, position.y + 38, 80, 15);
    ...
    public void update(float deltaTime) {
        if (isGo) {
            ...
            bodyFalcon.setPosition(position.x + 15, position.y + 38);
    ...
    public void renderHelp(SpriteBatch batch) {
        batch.draw(animationModels.getFrame(), 256, RuDragonGame.HEIGHT - 320);
    }

    @Override
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyFalcon) || pawsR11.overlaps(bodyFalcon) || pawsR12.overlaps(bodyFalcon) ||
                paws.overlaps(bodyFalcon) || head.overlaps(bodyFalcon) || wings.overlaps(bodyFalcon) ||
