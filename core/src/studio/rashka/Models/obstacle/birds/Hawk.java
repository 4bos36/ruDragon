...
public class Hawk implements Collides {
    ...
    private Rectangle bodyHawk, wings;
    ...
    public Hawk() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("hawk")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("hawk")), 17, 0.7f);
        ...
        bodyHawk = new Rectangle(position.x + 15, position.y + 38, 80, 15);
    ...
    public void update(float deltaTime) {
        if (isGo) {
            if (position.x < RuDragonGame.WIDTH && isSound) {
                if (RuDragonGame.getPreference().loadSound() == 1) RuDragonGame.getMusicSound().sounds.get("hawk").play();
                isSound = false;
            }
            ...
            bodyHawk.setPosition(position.x + 15, position.y + 38);
    ...
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyHawk) || pawsR11.overlaps(bodyHawk) || pawsR12.overlaps(bodyHawk) ||
                paws.overlaps(bodyHawk) || head.overlaps(bodyHawk) || wings.overlaps(bodyHawk) ||
