...
public class Korshun implements Collides {
    ...
    private Rectangle bodyKorshun, wings;
    ...
    public Korshun() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("korshun")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("korshun")), 17, 0.7f);
        ...
        bodyKorshun = new Rectangle(position.x + 15, position.y + 38, 80, 15);
    ...
    public void update(float deltaTime) {
        if (isGo) {
            if (position.x < RuDragonGame.WIDTH && isSound) {
                if (RuDragonGame.getPreference().loadSound() == 1) RuDragonGame.getMusicSound().sounds.get("korshun").play();
                isSound = false;
            }
            ...
            bodyKorshun.setPosition(position.x + 15, position.y + 38);
    ...
    public void renderHelp(SpriteBatch batch) {
        batch.draw(animationModels.getFrame(), RuDragonGame.WIDTH / 2, RuDragonGame.HEIGHT - 96);
    }

    @Override
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyKorshun) || pawsR11.overlaps(bodyKorshun) || pawsR12.overlaps(bodyKorshun) ||
                paws.overlaps(bodyKorshun) || head.overlaps(bodyKorshun) || wings.overlaps(bodyKorshun) ||
