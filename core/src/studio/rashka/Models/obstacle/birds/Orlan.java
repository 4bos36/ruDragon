...
public class Orlan implements Collides {
    ...
    private Rectangle bodyOrlan, wings;
    ...
    public Orlan() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("orlan")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("orlan")), 17, 0.7f);
        random = new Random();
        flyY();
        ...
        bodyOrlan = new Rectangle(position.x + 15, position.y + 38, 80, 15);
    ...
    public void update(float deltaTime) {
        if (isGo) {
            if (position.x < RuDragonGame.WIDTH && isSound) {
                if (RuDragonGame.getPreference().loadSound() == 1) RuDragonGame.getMusicSound().sounds.get("orlan").play();
                isSound = false;
            }
            ...
            bodyOrlan.setPosition(position.x + 15, position.y + 38);
    ...
    public void renderHelp(SpriteBatch batch) {
        batch.draw(animationModels.getFrame(), RuDragonGame.WIDTH - 128, 640);
    }

    @Override
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyOrlan) || pawsR11.overlaps(bodyOrlan) || pawsR12.overlaps(bodyOrlan) ||
                paws.overlaps(bodyOrlan) || head.overlaps(bodyOrlan) || wings.overlaps(bodyOrlan) ||
