...
public class Osprey implements Collides {
    ...
    private Rectangle bodyOsprey, wings;
    ...
    public Osprey() {
        animationModels = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("osprey")), 17, 1.0f);
        animationModelsFaster = new AnimationModels(new TextureRegion(RuDragonGame.getTextures().textureRegions.get("osprey")), 17, 0.7f);
        random = new Random();
        flyY();
        ...
        bodyOsprey = new Rectangle(position.x + 15, position.y + 38, 80, 15);
        wings = new Rectangle(position.x + 35, position.y + 84, 32, 28);
    }

    private int flyY() {
        posY = 200 + random.nextInt(RuDragonGame.HEIGHT / 2);
        return posY;
    }
    ...
    public void update(float deltaTime) {
        if (isGo) {
            if (position.x < RuDragonGame.WIDTH && isSound) {
                if (RuDragonGame.getPreference().loadSound() == 1) RuDragonGame.getMusicSound().sounds.get("osprey").play();
                isSound = false;
            }
            ...
            bodyOsprey.setPosition(position.x + 15, position.y + 38);
    ...
    public void homeUpdate(float deltaTime) {
        if (!isGo) {
            if (random.nextInt(100) == 0) isGo = true;
        }
        else if (isGo) {
            ...
            if (position.x + WIDTH < 0 || RuDragonGame.getPreference().isRoarDragon()) {
                isGo = false;
                isSound = true;
                position.x = START_POSITION;
                position.y = flyY();
    ...
    public boolean collides(Rectangle tail, Rectangle pawsR11, Rectangle pawsR12, Rectangle paws, Rectangle head, Rectangle wings) {
        return (tail.overlaps(bodyOsprey) || pawsR11.overlaps(bodyOsprey) || pawsR12.overlaps(bodyOsprey) ||
                paws.overlaps(bodyOsprey) || head.overlaps(bodyOsprey) || wings.overlaps(bodyOsprey) ||
