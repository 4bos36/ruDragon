...
public class Rock2 implements Collides {
    ...
    public Rock2() {
        ...
        body = new Circle(position.x + 124, 160, 114);
        bodyMini = new Rectangle(position.x + 56, 280, 90, 20);
    }

    public void update(float deltaTime) {
        if (isGo) {
            position.add(MOVEMENT * deltaTime * speed, 0);
            body.setPosition(position.x + 124, 160);
            bodyMini.setPosition(position.x + 56, 280);
    ...
    public void render(SpriteBatch batch) {
        if (isGo) batch.draw(RuDragonGame.getTextures().textureRegions.get("rock_2"), position.x, position.y);
    }
