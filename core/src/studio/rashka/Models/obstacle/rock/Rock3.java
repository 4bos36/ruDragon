...
public class Rock3 implements Collides {
    ...
    public Rock3() {
        ...
        body = new Circle(position.x + 124, 170, 114);
        bodyMini = new Rectangle(position.x + 110, 290, 100, 20);
    }

    public void update(float deltaTime) {
        if (isGo) {
            position.add(MOVEMENT * deltaTime * speed, 0);
            body.setPosition(position.x + 124, 170);
            bodyMini.setPosition(position.x + 110, 290);
    ...
    public void render(SpriteBatch batch) {
        if (isGo) batch.draw(RuDragonGame.getTextures().textureRegions.get("rock_3"), position.x, position.y);
    }
