...
public class Rock5 implements Collides {
    ...
    public Rock5() {
        ...
        body = new Circle(position.x + 130, 225, 96);
        bodyMini = new Rectangle(position.x + 72, 350, 112, 20);
    }

    public void update(float deltaTime) {
        if (isGo) {
            position.add(MOVEMENT * deltaTime * speed, 0);
            body.setPosition(position.x + 130, 225);
            bodyMini.setPosition(position.x + 72, 350);
    ...
    public void render(SpriteBatch batch) {
        if (isGo) batch.draw(RuDragonGame.getTextures().textureRegions.get("rock_5"), position.x, position.y);
    }
