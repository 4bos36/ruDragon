...
public class Rock6 implements Collides {
    ...
    public Rock6() {
        ...
        body = new Circle(position.x + 122, 215, 96);
        bodyMini = new Rectangle(position.x + 107, 343, 67, 20);
    }

    public void update(float deltaTime) {
        if (isGo) {
            position.add(MOVEMENT * deltaTime * speed, 0);
            body.setPosition(position.x + 122, 215);
            bodyMini.setPosition(position.x + 107, 343);
    ...
    public void render(SpriteBatch batch) {
        if (isGo) batch.draw(RuDragonGame.getTextures().textureRegions.get("rock_6"), position.x, position.y);
    }
